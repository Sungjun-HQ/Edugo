...
<%
	...
	String sql = "select f.faq_idx, f.faq_title, f.faq_content,"
			+ " f.faq_date, f.faq_count, u.user_id"
			+ " from user_table u, faq f "
			+ " where u.user_idx=f.user_idx and f.faq_idx=?";
	...
	String sql2 = "update faq set faq_count=? where faq_idx=?";
		...
		<div class="row">
			...
			<div class="body col-md-9">
				...
				<div class="bodyinborder">
					...
					<div class="right">
						<a href="<%=path%>/help/faq.jsp?page_num=<%=page_num%>" class="btn btn-default">목록보기</a>
						<%-- 작성자와 로그인한 사람이 같을 경우 나타나게 한다. --%>
						<%
							if (user_id.equals(user_bean.getUser_id())) {
						%>
						<a href="<%=path%>/help/faq_modify.jsp?board_idx=<%=board_idx%>&page_num=<%=page_num%>" class="btn btn-default">수정</a>
						<a href="<%=path%>/board/board_delete_pro.jsp?board_idx=<%=board_idx%>&board_num=6" class="btn btn-default">삭제</a>
						<%
							}
						%>
					</div>

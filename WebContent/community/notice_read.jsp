...
<%
	...
	// index 번째 글 데이터를 추출
	String sql = "select n.notice_idx, n.notice_title, n.notice_content,"
			+ " n.notice_date, n.notice_count, u.user_id"
			+ " from user_table u, notice n "
			+ " where u.user_idx=n.user_idx and n.notice_idx=?";
	...
	// 기타 등등...
	String sql2 = "update notice set notice_count=? where notice_idx=?";
		...
		<div class="row">
			...
			<div class="body col-md-9">
				...
				<div class="bodyinborder">
					...
					<div class="right">
						<a href="<%=path%>/community/notice.jsp?page_num=<%=page_num%>" class="btn btn-default">목록보기</a>
						
						<%-- 작성자와 로그인한 사람이 같을 경우 나타나게 한다. --%>
						<%
							if (user_id.equals(user_bean.getUser_id())) {
						%>
						<a href="<%=path%>/community/notice_modify.jsp?board_idx=<%=board_idx%>&page_num=<%=page_num%>" class="btn btn-default">수정</a>
						<a href="<%=path%>/board/board_delete_pro.jsp?board_idx=<%=board_idx%>&board_num=1" class="btn btn-default">삭제</a>
						<%
							}
						%>
					</div>

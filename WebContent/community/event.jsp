...
<%
	...
	// 3: 이벤트
	ArrayList<HashMap<String, Object>> event_list = BoardDAO.getBoard(page_num, 3);

	String sql2 = "select count(*) from event";
...
<script type="text/javascript">
	...
	$(function(){
		$("#board_list tbody tr").click(function(){
			...
			// 이동한다.
			location.href = "<%=path2%>/community/event_read.jsp?board_idx=" + idx + "&page_num=" + <%=page_num%>;
		});
		// 페이지 번호 셋팅
			...
			<div class="body col-md-9">
				...
				<div class="bodyinborder">
					...
					<table class="table table-hover" id="board_list">
						...
						<tbody>
						<%
							for (HashMap<String, Object> map : event_list) {
								date = (String)map.get("board_date");
								date = date.substring(0, 10);
						%>
							<tr data-idx="<%=map.get("board_idx")%>">
								<td class="center"><%=map.get("board_idx")%></td>
								<td><a href="<%=path%>/community/event_read.jsp?board_idx=<%=map.get("board_idx")%>&page_num=<%=page_num%>"><%=map.get("board_title")%></a></td>
					...
					<div class="right">
						<%
							if (chk_login!=null) {
						%>
						<a href="<%=path%>/community/event_write.jsp" class="btn btn-default">글쓰기</a>
						<%
							}
						%>
					</div>

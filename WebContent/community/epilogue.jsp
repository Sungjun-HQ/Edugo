...
<%
	...
	// 2: 수강후기
	ArrayList<HashMap<String, Object>> epilogue_list = BoardDAO.getBoard(page_num, 2);

	// 전체 게시글의 갯수를 구한다.
	// epilogue 테이블(수강후기 테이블)의 갯수를 구해오는 쿼리문
	String sql2 = "select count(*) from epilogue";
	// Statement 생성
...
<script type="text/javascript">
	...
	$(function(){
		$("#board_list tbody tr").click(function(){
			...
			// 이동한다.
			location.href = "<%=path2%>/community/epilogue_read.jsp?board_idx=" + idx + "&page_num=" + <%=page_num%>;
		});
		// 페이지 번호 셋팅
			...
			<div class="body col-md-9">
				...
				<div class="bodyinborder">
					<table class="table table-hover" id="board_list">
						...
						<tbody>
							<%
								for (HashMap<String, Object> map : epilogue_list) {
									date = (String)map.get("board_date");
									date = date.substring(0, 10);
							%>
							<tr data-idx="<%=map.get("board_idx")%>">
								<td class="center"><%=map.get("board_idx")%></td>
								<td><a href="<%=path%>/community/epilogue_read.jsp?board_idx=<%=map.get("board_idx")%>&page_num=<%=page_num%>"><%=map.get("board_title")%></a></td>
					...
					<div class="center">
						...
						<ul class="pagination pagination-sm" id="page_indicator">
							...
							%>
							<li>
								<a href="<%=path%>/community/epilogue.jsp?page_num=<%=i + 1%>"><%=i + 1%></a>
							</li>
							<%
								}
							%>
					...
					<div class="right">
						<%
							if (chk_login!=null) {
						%>
						<a href="<%=path%>/community/epilogue_write.jsp" class="btn btn-default">글쓰기</a>
						<%
							}
						%>
					</div>

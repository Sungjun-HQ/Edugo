...
<%@ include file="/header_footer/header_next.jsp"%>
		...
		<div class="row">
			...
			<div class="body col-md-9">
				...
				<h3>공지사항 > READ > MODIFY</h3>
				<br />
				
				<div class="bodyinborder">
				
					<form action="<%=path%>/board/board_modify_pro.jsp?board_idx=<%=board_bean.getBoard_idx()%>&page_num=<%=page_num%>&board_num=1" method="post">
					
						<div class="form-group">
							<label for="board_title">제목 : </label> 
							<input type="text" class="form-control" name="board_title" value="<%=rs.getString("notice_title")%>" />
						</div>
						
						<div class="form-group">
							<label for="board_content">내용 : </label>
							<textarea class="form-control" name="board_content" rows="10"><%=rs.getString("notice_content")%></textarea>
						</div>
						...
						<div class="right">
							...
							<a href="<%=path%>/community/notice_read.jsp?board_idx=<%=board_bean.getBoard_idx()%>&page_num=<%=page_num%>" class="btn btn-default">이전으로</a>
						</div>
					</form>
